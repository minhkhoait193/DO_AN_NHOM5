...
# Lấy danh sách các ID xe không trùng lặp
    for car_id in unique_car_ids: # rồi lặp qua từng xe này
...
# Lấy danh sách số khung hình từ dữ liệu
        car_frame_numbers = frame_numbers[car_mask]#car_frame_numbers là các frame xe này xuất hiện (đã lọc)
        car_bboxes_interpolated = []# tạo biến để lưu
        license_plate_bboxes_interpolated = []# tạo biến để lưu
...
# Nếu có khoảng trống giữa 2 frame → thực hiện nội suy
                if frame_number - prev_frame_number > 1:
                    ...
                    #Tính frame ở giữa như: 3.0, 4.0 nếu khoảng cách từ frame 2 đến 5.
...
# Lấy tọa độ khung biển số xe
                    ...
                    #Không lấy frame đầu tiên (đã có rồi), chỉ lấy những cái nội suy ở giữa.
                    car_bboxes_interpolated.extend(interpolated_car_bboxes[1:])
                    license_plate_bboxes_interpolated.extend(interpolated_license_plate_bboxes[1:])

# Thêm bounding box vào danh sách kết quả (dù nội suy hay không)
...
# Nếu là frame được nội suy → set điểm số và nội dung = 0
            if str(frame_number) not in frame_numbers_:
                ...
                # Nếu là dữ liệu gốc → lấy đúng giá trị gốc
